...
(function zanimation($) {
  ...
  const getController = (el) => {
      ...
      if (value.hasAttribute(`data-zanim-${breakPointConst.currentScrean}`)) {
        ...
        $.each(value.attributes, (i, attribute) => {
          if (
            ...
            const breakPoint =
              utils.breakpoints[attribute.name.split("data-zanim-")[1]];
    ...
    //(controller.ease && (controller.to.ease = controller.ease) &&
      //controller.to.ease) ||
  ...
  jQuery.fn.zanimation = function zanim(callback) {
    ...
    if (zanimTimeline) {
      const timeline = gsap.timeline(zanimTimeline);
      const timelineElements = $this.find(
